...
class ConsultaTest {
 ...
 @Test void filtraCombinaPaginaEComparaFusos() throws Exception {
  ...
  try {
   ...
   assertEquals("B",pagina.getAsJsonArray("leituras").get(0).getAsJsonObject().get("area").getAsString());
   assertEquals(1,json(servidor,"?sensor=A&data=2026-09-18&percentual_ocupado_min=10&percentual_ocupado_max=10").get("total").getAsInt());
   assertEquals(2,json(servidor,"?data_inicio=2026-09-18T15:00:00Z&data_fim=2026-09-18T15:00:00Z").get("total").getAsInt());
   assertEquals(1,json(servidor,"?percentual_ocupado=50").get("total").getAsInt());
   ...
   try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement()) {
    sql.execute("DROP TABLE leituras_manancial");
   }
 ...
 @Test void rejeitaFiltrosInvalidos() throws Exception {
  ...
  try {
   for(String query: new String[]{"?inexistente=1","?sensor=","?sensor=A&sensor=B","?data=2026-02-30","?data_inicio=ontem","?data_inicio=2026-09-19T00:00:00Z&data_fim=2026-09-18T00:00:00Z","?percentual_ocupado_min=90&percentual_ocupado_max=10","?percentual_ocupado=NaN","?percentual_ocupado_max=Infinity","?limite=0","?limite=1001","?offset=-1"})
 ...
 private HttpResponse<String> get(HttpServer s,String query) throws Exception {
  return HttpClient.newHttpClient().send(HttpRequest.newBuilder(URI.create("http://127.0.0.1:"+s.getAddress().getPort()+"/manancial/historico"+query)).GET().build(),HttpResponse.BodyHandlers.ofString());
 }
