...
class RuntimeTest {
 @TempDir Path pasta;
 static final String CORPO="{\"area\":\"PONTO-TESTE\",\"timestamp\":\"2026-09-24T15:00:00Z\",\"percentual_ocupado\":10,\"nivel_agua_m\":2,\"temperatura_agua_c\":20}";
 ...
 @Test void sseEAlertasContinuamComBancoBloqueado() throws Exception {
  ...
  String base="http://127.0.0.1:"+servidor.getAddress().getPort()+"/manancial";
  try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement()) {
   ...
   try(var stream=resposta.body()) {
    ...
    assertEquals(0,contar(arquivo,"leituras_manancial"));
    sql.execute("ROLLBACK");
    aguardar(()->contar(arquivo,"leituras_manancial")==1);
 ...
 @Test void processamentoDeFilaEhIdempotente() throws Exception {
  ...
  try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement();Fluxo fluxo=new Fluxo(banco,arquivo)) {
   ...
   aguardar(()->contar(arquivo,"leituras_manancial")==1);
   assertEquals(1,contar(arquivo,"alertas_manancial"));
  }
 }
