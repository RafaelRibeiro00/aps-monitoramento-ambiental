...
final class Migracao {
    static void aplicar(String url) throws SQLException {
        try(Connection c=DriverManager.getConnection(url);var s=c.createStatement()) {
            ...
            try {
                if("alagamento".equals("alagamento")) reconstruir(c,"leituras_alagamento",false);
                reconstruir(c,"alertas_alagamento",true);
                adicionar(c,"leituras_alagamento","evento_id","TEXT");
                adicionar(c,"alertas_alagamento","evento_id","TEXT");
                s.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_leituras_alagamento_evento ON leituras_alagamento(evento_id)");
                s.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_alertas_alagamento_evento ON alertas_alagamento(evento_id)");
                s.execute("CREATE INDEX IF NOT EXISTS idx_alertas_alagamento_envio ON alertas_alagamento(status_notificacao,proxima_tentativa)");
                if("alagamento".equals("alagamento")) {
                    adicionar(c,"leituras_alagamento","ponto_id","TEXT");
                    adicionar(c,"leituras_alagamento","status","TEXT");
                    s.execute("UPDATE leituras_alagamento SET ponto_id=sensor WHERE ponto_id IS NULL");
                    s.execute("UPDATE leituras_alagamento SET status=CASE WHEN nivel_corrego_cm IS NULL OR chuva_mm IS NULL OR velocidade_agua_m_s IS NULL THEN 'incompleta' ELSE 'completa' END WHERE status IS NULL");
