...
public record Consulta(String where, List<Object> valores, int limite, int offset) {
    ...
    public static Consulta deQuery(String query, boolean alertas) {
        ...
        String identificador = parametros.get("sensor");
        if (!"sensor".equals("sensor") && identificador != null && parametros.containsKey("sensor"))
            throw new IllegalArgumentException("Use sensor ou sensor, nao ambos.");
        if (identificador == null) identificador = parametros.get("sensor");
        if (identificador != null) adicionar(condicoes, valores, "sensor = ?", identificador);
