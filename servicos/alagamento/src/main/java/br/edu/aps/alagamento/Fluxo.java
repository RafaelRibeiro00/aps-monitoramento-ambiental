...
final class Fluxo implements AutoCloseable {
    ...
    private static Thread daemon(Runnable r,String nome){Thread t=new Thread(r,"alagamento-"+nome);t.setDaemon(true);return t;}

    Fluxo(Banco banco,Path arquivo) throws IOException {
        this.banco=banco;
        Path pasta=arquivo.toAbsolutePath().resolveSibling(arquivo.getFileName()+".fila-alagamento");
    ...
    synchronized Map<String,Object> receber(String corpo) throws IOException {
        ...
        if("alagamento".equals("alagamento"))return receberMedicao(corpo);
    ...
    private JsonArray alertas(Leitura leitura,JsonObject evento) {
        ...
        a.addProperty("servico","alagamento");a.addProperty("criado_em",evento.get("recebido_em").getAsString());a.add("leitura",evento.get("leitura").deepCopy());lista.add(a);return lista;
    }
