...
public final class Notificador implements AutoCloseable {
    ...
    private final ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "notificacoes-alagamento"); t.setDaemon(true); return t;
    });

    public Notificador(Banco banco) {
        this(banco, Configuracao.valor("ALERTA_WEBHOOK_URL", Configuracao.valor("ALAGAMENTO_WEBHOOK_URL", "")), Configuracao.valor("ALERTA_WEBHOOK_TOKEN", Configuracao.valor("ALAGAMENTO_WEBHOOK_TOKEN", "")));
    }
            ...
            for (int i = 0; i < 10 && !Thread.currentThread().isInterrupted(); i++) {
                ...
                try {
                    var pedido = HttpRequest.newBuilder(destino).timeout(Duration.ofSeconds(5))
                            ...
                            .header("Idempotency-Key", "aps-alagamento-" + id)
