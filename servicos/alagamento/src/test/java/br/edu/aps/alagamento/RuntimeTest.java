...
class RuntimeTest {
 @TempDir Path pasta;
 static final String CORPO="{\"sensor_id\":\"N-1\",\"ponto_id\":\"PONTO-TESTE\",\"tipo\":\"nivel_corrego\",\"valor\":210,\"unidade\":\"cm\",\"timestamp\":\"2026-09-24T15:00:00Z\"}";
 ...
 @Test void sseEAlertasContinuamComBancoBloqueado() throws Exception {
  ...
  String base="http://127.0.0.1:"+servidor.getAddress().getPort()+"/alagamento";
  try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement()) {
   ...
   try(var stream=resposta.body()) {
    ...
    assertEquals(0,contar(arquivo,"leituras_alagamento"));
    sql.execute("ROLLBACK");
    aguardar(()->contar(arquivo,"leituras_alagamento")==1);
 ...
 @Test void processamentoDeFilaEhIdempotente() throws Exception {
  ...
  try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement();Fluxo fluxo=new Fluxo(banco,arquivo)) {
   ...
   aguardar(()->contar(arquivo,"medicoes_alagamento")==1);
   assertEquals(1,contar(arquivo,"alertas_alagamento"));
  }
 }
