...
class ConsultaTest {
 ...
 @Test void filtraCombinaPaginaEComparaFusos() throws Exception {
  ...
  try {
   ...
   assertEquals("B",pagina.getAsJsonArray("leituras").get(0).getAsJsonObject().get("sensor").getAsString());
   assertEquals(1,json(servidor,"?sensor=A&data=2026-09-18&nivel_corrego_cm_min=10&nivel_corrego_cm_max=10").get("total").getAsInt());
   assertEquals(2,json(servidor,"?data_inicio=2026-09-18T15:00:00Z&data_fim=2026-09-18T15:00:00Z").get("total").getAsInt());
   assertEquals(1,json(servidor,"?nivel_corrego_cm=50").get("total").getAsInt());
 ...
 @Test void rejeitaFiltrosInvalidos() throws Exception {
  ...
  try {
   for(String query: new String[]{"?inexistente=1","?sensor=","?sensor=A&sensor=B","?data=2026-02-30","?data_inicio=ontem","?data_inicio=2026-09-19T00:00:00Z&data_fim=2026-09-18T00:00:00Z","?nivel_corrego_cm_min=90&nivel_corrego_cm_max=10","?nivel_corrego_cm=NaN","?nivel_corrego_cm_max=Infinity","?limite=0","?limite=1001","?offset=-1"})
