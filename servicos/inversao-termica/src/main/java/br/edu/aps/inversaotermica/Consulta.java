...
public record Consulta(String where, List<Object> valores, int limite, int offset, String ordem) {
    private static final List<String> MEDIDAS = List.of("umidade", "temperatura_c", "vento_km_h");
    ...
    public static Consulta deQuery(String query, boolean alertas) {
        ...
        Set<String> permitidos = new HashSet<>(List.of("sensor", "estacao", "data", "data_inicio", "data_fim", "limite", "offset", "ordem"));
        ...
        String identificador = parametros.get("estacao");
        if (!"sensor".equals("estacao") && identificador != null && parametros.containsKey("sensor"))
            throw new IllegalArgumentException("Use sensor ou estacao, nao ambos.");
        if (identificador == null) identificador = parametros.get("sensor");
        if (identificador != null) adicionar(condicoes, valores, "estacao = ?", identificador);
