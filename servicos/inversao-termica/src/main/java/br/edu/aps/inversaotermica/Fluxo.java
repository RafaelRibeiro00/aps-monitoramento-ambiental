...
final class Fluxo implements AutoCloseable {
    ...
    private static Thread daemon(Runnable r,String nome){Thread t=new Thread(r,"inversao-termica-"+nome);t.setDaemon(true);return t;}

    Fluxo(Banco banco,Path arquivo) throws IOException {
        this.banco=banco;
        Path pasta=arquivo.toAbsolutePath().resolveSibling(arquivo.getFileName()+".fila-inversao-termica");
    ...
    synchronized Map<String,Object> receber(String corpo) throws IOException {
        ...
        if("inversao-termica".equals("alagamento"))return receberMedicao(corpo);
    ...
    private JsonArray alertas(Leitura leitura,JsonObject evento) {
        ...
        a.addProperty("servico","inversao-termica");a.addProperty("criado_em",evento.get("recebido_em").getAsString());a.add("leitura",evento.get("leitura").deepCopy());lista.add(a);return lista;
    }
