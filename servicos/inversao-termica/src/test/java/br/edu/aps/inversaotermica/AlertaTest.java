...
class AlertaTest {
 @TempDir Path pasta;
 private Leitura critica() { return new Leitura("SENSOR-TESTE", "2026-09-18T15:00:00Z", 30.0, 30.0, 5.0); }
 @Test void geraSomenteQuandoNecessarioEPersisteComALeitura() throws Exception {
  ...
  assertTrue(((java.util.List<?>)banco.salvar(new Leitura("NORMAL", "2026-09-18T15:00:00Z", 50.0, 20.0, 10.0)).get("alertas")).isEmpty());
  ...
  try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement()) { sql.execute("DROP TABLE alertas_inversaotermica"); }
  Banco mesmo=banco;
 ...
 @Test void webhookRepeteFalhaSemDuplicarLeituraOuAlerta() throws Exception {
  ...
  try(var n=new Notificador(banco,"http://127.0.0.1:"+destino.getAddress().getPort()+"/notificacoes","")) {
   ...
   try(var c=DriverManager.getConnection("jdbc:sqlite:"+arquivo);var sql=c.createStatement()) {sql.execute("UPDATE alertas_inversaotermica SET proxima_tentativa=0");}
   ...
   assertEquals("inversao-termica",JsonParser.parseString(corpo.get()).getAsJsonObject().get("servico").getAsString());
